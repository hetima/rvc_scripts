...
(function () {
    ...
    const jpDict = {
        ...
        "step2a：自动遍历训练文件夹下所有可解码成音频的文件并进行切片归一化，在实验目录下生成2个wav文件夹；暂时只支持单人训练。": "トレーニングフォルダ内のすべての音声ファイルを自動的に処理し、スライスと正規化を行います。処理後のファイルは、logsフォルダ内に生成されたフォルダに保存されます。ただし、現在は、単一話者のみサポートされています。",
